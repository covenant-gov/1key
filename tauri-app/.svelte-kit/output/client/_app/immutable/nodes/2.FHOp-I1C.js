import{f as m,a as f}from"../chunks/sZaBqZ7P.js";import"../chunks/BnMwdpAs.js";import{o as I}from"../chunks/BEEfpOzW.js";import{p as y,j as g}from"../chunks/BykR2jw5.js";import{i as E}from"../chunks/D4NwpSHk.js";class d{constructor(t,e={}){this.container=document.getElementById(t),this.options={length:6,onComplete:null,onInput:null,placeholder:"•",...e},this.boxes=[],this.currentIndex=0}render(){this.container.innerHTML=`
    ...
    `,this.initPinInput(),this.setupEventListeners()}initPinInput(){this.pinInput=new d("pin-input-container",{length:6,onComplete:t=>{this.handleUnlock(t)},onInput:(t,e)=>{this.hideError();const n=document.getElementById("unlock-btn");t.length>0&&n&&(n.style.display="block")}}),this.pinInput.render()}setupEventListeners(){document.getElementById("unlock-btn").addEventListener("click",()=>{const e=this.pinInput.getPinValue();this.pinInput.isComplete()&&this.handleUnlock(e)}),document.addEventListener("keydown",e=>{if(e.key==="Enter"&&this.pinInput&&this.pinInput.isComplete()){const n=this.pinInput.getPinValue();this.handleUnlock(n)}})}async handleUnlock(t){if(this.isLocked)return;if(!t||t.length!==6){this.showError("Please enter a 6-digit PIN"),this.pinInput.shake();return}this.pinInput.disable();const e=document.getElementById("unlock-btn");e&&(e.disabled=!0,e.textContent="Unlocking...");try{this.onUnlock&&(await this.onUnlock(t),this.attempts=0,this.hideError(),this.hideAttemptsInfo())}catch{this.handleUnlockFailure()}}handleUnlockFailure(){this.attempts++;const t=this.maxAttempts-this.attempts;this.pinInput.enable();const e=document.getElementById("unlock-btn");e&&(e.disabled=!1,e.textContent="Unlock"),this.attempts>=this.maxAttempts?this.lockOut():(this.showError(`Incorrect PIN. ${t} attempt${t!==1?"s":""} remaining.`),this.showAttemptsInfo(t),this.pinInput.shake(),this.pinInput.clear(),setTimeout(()=>{this.pinInput.focusBox(0)},100))}lockOut(){this.isLocked=!0,this.pinInput.disable();const t=document.getElementById("unlock-btn");t&&(t.disabled=!0,t.style.display="none");const e=document.getElementById("lockout-message");e&&(e.textContent=`Too many failed attempts. Please wait ${this.lockoutTime/1e3} seconds before trying again.`,e.style.display="block"),this.hideError(),this.hideAttemptsInfo();let n=this.lockoutTime/1e3;const s=setInterval(()=>{n--,e&&(e.textContent=`Too many failed attempts. Please wait ${n} second${n!==1?"s":""} before trying again.`),n<=0&&(clearInterval(s),this.unlock())},1e3)}unlock(){this.isLocked=!1,this.attempts=0,this.pinInput.enable();const t=document.getElementById("unlock-btn");t&&(t.disabled=!1,t.style.display="block",t.textContent="Unlock");const e=document.getElementById("lockout-message");e&&(e.style.display="none"),this.pinInput.clear(),this.pinInput.focusBox(0)}showError(t){const e=document.getElementById("unlock-error");e&&(e.textContent=t,e.style.display="block"),this.pinInput&&this.pinInput.showError(t)}hideError(){const t=document.getElementById("unlock-error");t&&(t.style.display="none"),this.pinInput&&this.pinInput.hideError()}showAttemptsInfo(t){const e=document.getElementById("attempts-remaining");e&&(e.textContent=`${t} attempt${t!==1?"s":""} remaining`,e.style.display="block")}hideAttemptsInfo(){const t=document.getElementById("attempts-remaining");t&&(t.style.display="none")}reset(){this.attempts=0,this.isLocked=!1,this.pinInput&&(this.pinInput.clear(),this.pinInput.enable());const t=document.getElementById("unlock-btn");t&&(t.disabled=!1,t.textContent="Unlock",t.style.display="none"),this.hideError(),this.hideAttemptsInfo();const e=document.getElementById("lockout-message");e&&(e.style.display="none")}}const{invoke:u}=window.__TAURI__.core;class w{constructor(){this.isUnlocked=!1,this.walletData=null}async walletExists(){try{return await u("wallet_exists")}catch(t){return console.error("Error checking wallet existence:",t),!1}}async createWallet(t){if(!t||t.length!==6)throw new Error("PIN must be 6 digits");const e={private_key:this.generatePlaceholderPrivateKey(),address:this.generatePlaceholderAddress()};try{return await u("encrypt_and_store_wallet",{wallet:e,pin:t}),this.walletData={address:e.address},this.isUnlocked=!0,{address:e.address}}catch(n){throw console.error("Error creating wallet:",n),new Error("Failed to create wallet. Please try again.")}}async unlockWallet(t){if(this.isUnlocked&&this.walletData)return this.walletData;if(!t||t.length!==6)throw new Error("PIN must be 6 digits");try{const e=await u("decrypt_wallet_with_pin",{pin:t});return this.walletData=e,this.isUnlocked=!0,e}catch(e){throw console.error("Error unlocking wallet:",e),e.toString().includes("Decryption failed")||e.toString().includes("Wrong PIN")?new Error("Incorrect PIN"):new Error("Failed to unlock wallet. Please try again.")}}lockWallet(){this.walletData=null,this.isUnlocked=!1}getWalletData(){return!this.isUnlocked||!this.walletData?null:this.walletData}isWalletUnlocked(){return this.isUnlocked}async deleteWallet(){try{await u("delete_wallet"),this.lockWallet()}catch(t){throw console.error("Error deleting wallet:",t),new Error("Failed to delete wallet")}}generatePlaceholderPrivateKey(){const t="0123456789abcdef";let e="";for(let n=0;n<64;n++)e+=t[Math.floor(Math.random()*t.length)];return"0x"+e}generatePlaceholderAddress(){const t="0123456789abcdef";let e="";for(let n=0;n<64;n++)e+=t[Math.floor(Math.random()*t.length)];return"0x"+e}}const h=new w;var P=m('<div id="app-container"><div class="loading-state"><h1>1Key</h1> <p>Loading...</p></div></div>');function S(l,t){y(t,!1);let e=!1;I(async()=>{e=await r(),e?a():n()});function n(){const i=new k("app-container");i.render(),i.onCreateAccount=async()=>{s()}}function s(){const i=new b("app-container");i.render(),i.onComplete=async c=>{try{console.log("Creating wallet with PIN:",c);const o=await h.createWallet(c);console.log("Wallet created successfully:",o.address),alert(`Account created successfully!

Address: ${o.address.substring(0,10)}...

Next time you open the app, you'll see the unlock screen.`),e=!0,a()}catch(o){console.error("Failed to create account:",o),i.showError(o.message||"Failed to create account. Please try again."),i.enableInputs(),i.reset()}}}function a(){const i=new v("app-container");i.render(),i.onUnlock=async c=>{try{console.log("Unlocking wallet with PIN:",c);const o=await h.unlockWallet(c);console.log("Wallet unlocked successfully:",o.address),alert(`Wallet unlocked!

Address: ${o.address.substring(0,10)}...

Password manager will be implemented in Step 7.`),i.reset()}catch(o){throw console.error("Failed to unlock:",o),o}}}async function r(){try{return await h.walletExists()}catch(i){return console.error("Error checking account existence:",i),!1}}E();var p=P();f(l,p),g()}export{S as component};
